...
export const paraphrase = async (userText: string) => {
	...
	if (userText !== "") {
		const { data } = await axios.post(`${URLS.urls.main}/api/utils/paraphrase`, {
...
export const summarize = async (userText: string) => {
	if (userText !== "") {
		const { data } = await axios.post(`${URLS.urls.main}/api/utils/summarize`, {
...
export const pdf = async function (text:string) {
	const apiKey = process.env.NEXT_PUBLIC_APITEMPLATE_API_KEY;
	const templateId = process.env.NEXT_PUBLIC_TEMPLATEID_API_KEY;
  ...
  export const complete = async (userText: string) => {
	if (userText !== "") {
		const { data } = await axios.post(`${URLS.urls.main}/api/utils/complete`, {
