...
import { validate_reset } from "@/app/lib/formVaild";
...
const SignInPage = () => {
  ...
  const [state, setState] = useState('Send Reset Link')
  ...
  const handleCredentialsSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null); // Clear previous errors
     if (
        ...
        setState("Send Reset Link")
        setIsNotif(true)
      } else {
            setState("Sending Reset Link")
  ...
  const handleBlur = (e: unknown) => {
    setMessage(validate_reset(formData));
  };
  return (
    <div 
    ...
    className="flex min-h-screen w-full items-center justify-center p-4">
      ...
      <div className="w-full max-w-md space-y-3">
        ...
        onSubmit={handleCredentialsSubmit} className="space-y-4 bg-white p-8">
            ...
            {/* Submit Button */}
            <button
            disabled={state==="Sending Reset Link"}
                type="submit"
