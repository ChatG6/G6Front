...
export async function savedocs(type: string,link : string,status:string,total_pages:Number,date:string,name:string,retries = 3, delay = 500) {
  try {
     const docs = await axios.post(
       URLS.endpoints.documents,
       {type:type,link:link,status:status,total_pages:total_pages,date:date,name:name}
     );
     console.log(docs);
     ...
     if (retries > 0) {
       ...
       await sleep(delay);
       // Retry the request with one less retry attempt and double the delay
       return savedocs(type,link,status,total_pages, date,name,retries - 1, delay);
     } else {
       // If no more retries left, throw the error
       throw error;
     }
 ...
 export async function archive(fileName:string,retries = 3, delay = 500) {
  try {
  const docs = await axios.post(
    URLS.endpoints.archive,
    {name:fileName});
     if(docs) return {data:{success:true,date:docs.data.date}};
     return {data:{success:false,date:''}}
   } catch (error) {
     if (retries > 0) {
       // Wait for the specified delay before retrying
       await sleep(delay);
 ...
 export async function uploadpdf(file:any,fileName:string,retries = 3, delay = 500) {
  //const docs = await axios.post(
   // URLS.endpoints.upload,
    //{name:fileName});
    fileName = 'y';
 ...
 export async function update(file: any, fileName: string, retries = 3, delay = 500) {
  ...
  try {
    ...
    if (retries > 0) {
      ...
      return { data: { success: false } };
      // Retry the request with one less retry attempt and double the delay
      // return upload(file, fileName, retries - 1, delay * 2);
    } else {
      // If no more retries left, throw the error
      throw error;
    }
 ...
 */
 ...
 export async function importpdf(fileName:string, retries = 3, delay = 500) {
  //console.log('N');
  //console.log(fileName);
 // const docs = await axios.post(
   // URLS.endpoints.get,
  ...
  try {
     ...
     const response = await client.send(command);
     // Ensure to await the transformation of the stream to a string
     const htmlContent = (await response.Body?.transformToByteArray())?.buffer;
     if (htmlContent) {
       return { data: { message: htmlContent } };
     } else {
       return { data: { message: "Something went wrong" } };
